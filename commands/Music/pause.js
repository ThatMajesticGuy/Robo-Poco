...
if (!message.member.permissions.has("ADMINISTRATOR")) return message.channel.send("You do not have permission to run this command")
   ...
   if (!voiceChannel) return message.channel.send(`You need to be in a voice channel to pause!`) 

if (serverQueue && serverQueue.playing) {
      serverQueue.playing = false;
      serverQueue.connection.dispatcher.pause();
      return message.channel.send(":radio_button: Paused the music for you!");
    }
    return message.channel.send("I can't pause Nothing?");
